...
- `Core/Processors`: `ChannelMapOptions` + `ColorChannel`, and a new `ProcessorOrder.ChannelMap` (50).
- `Imaging/Processors/ChannelMapProcessor` — byte-level channel moves, stride-aware.
- `UI`: a "Channels" tab with one combo box per output channel.
- **Automated verification:** full suite green on both platforms; both AOT publishes clean.
...
- `Core/Processors/InvertOptions`, and a new `ProcessorOrder.Invert` (75).
- `Imaging/Processors/InvertProcessor` — byte-level, premultiplication-correct, stride-aware.
- `UI`: an "Invert" tab with a single checkbox.
- **Automated verification:** full suite green on both platforms; both AOT publishes clean.
